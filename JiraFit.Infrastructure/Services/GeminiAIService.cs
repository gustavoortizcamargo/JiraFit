    ...
    public async Task<Result<NutritionalAnalysisDto>> AnalyzeMealAsync(MealInputDto input, JiraFit.Domain.Entities.User currentUser, CancellationToken cancellationToken = default)
    {
        try
        {
            ...
            var behaviorLogic = input.IsSuggestionRequest 
                ...
                : "SE a mensagem contiver comida (refeição), extraia os macros no formato JSON: 'Calories', 'Proteins', 'Carbs', 'Fats'. SE O usuário ditar a hora da refeição, extraia para 'MealHour' (0-23) e 'MealMinute' (0-59). ALÉM DISSO, preencha a chave 'Feedback' (string) avisando que a refeição foi salva. AGORA, SE a mensagem solicitar a criação de um ALARME, preencha ESTRITAMENTE: 'AlarmName' (string), 'AlarmHour' (número), 'AlarmMinute' (número). Confirme no 'Feedback'.";
