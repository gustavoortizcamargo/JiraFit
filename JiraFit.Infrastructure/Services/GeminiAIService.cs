...
public class GeminiAIService : IAIService
{
    ...
    public async Task<Result<NutritionalAnalysisDto>> AnalyzeMealAsync(MealInputDto input, JiraFit.Domain.Entities.User currentUser, CancellationToken cancellationToken = default)
    {
        try
        {
            ...
            var systemPrompt = "Você é o JiraFit, um assistente nutricional via WhatsApp muito amigável e empolgante. O usuário pode enviar áudios, imagens ou textos. " +
                               userContext +
                               "SE a mensagem contiver comida (refeição), extraia rigorosamente os macros no formato JSON: 'Calories' (número), 'Proteins' (número), 'Carbs' (número), 'Fats' (número). ALÉM DISSO, preencha obrigatoriamente a chave 'Feedback' (string) com um elogio amigável, rápido e motivacional sobre a refeição inserida. AGORA, SE a mensagem do usuário for ele dizendo o próprio NOME, PESO (kg) ou ALTURA (cm) sem mencionar comida, você NÃO extrai macros, mas coloca os dados que percebeu nas propriedades do JSON: 'ExtractedName' (string), 'ExtractedWeight' (número), 'ExtractedHeight' (número). Se houver dados básicos faltantes (ex: não tem nome ou peso ainda), fale gentilmente na chave 'Feedback' (string) pedindo essas informações. Retorne apenas e estritamente um objeto JSON.";
