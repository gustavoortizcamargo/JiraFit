    ...
    public async Task<Result<NutritionalAnalysisDto>> AnalyzeMealAsync(MealInputDto input, JiraFit.Domain.Entities.User currentUser, CancellationToken cancellationToken = default)
    {
        try
        {
            ...
            var behaviorLogic = input.IsSuggestionRequest 
                ? "IMPORTANTE: O usuário pede uma SUGESTÃO de jantar/refeição. Como nutricionista gatinho, elabore a receita DENTRO da chave 'Feedback' (string)." 
                : "SE a mensagem contiver comida (refeição), extraia os macros ('Calories', 'Proteins', 'Carbs', 'Fats'). INCLUA SEMPRE 'MealHour' (inteiro, 0-23 ou null se nulo) e 'MealMinute' (inteiro, 0-59 ou null). Preencha na chave 'Feedback' (string) um aviso que a refeição foi salva. AGORA, SE solicitar ALARME, preencha ESTRITAMENTE: 'AlarmName' (string), 'AlarmHour' (número), 'AlarmMinute' (número).";
